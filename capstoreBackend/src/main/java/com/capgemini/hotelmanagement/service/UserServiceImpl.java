...
public class UserServiceImpl implements UserService {
			...
			if (hotelValidation.passwordValidation(password)) {
				if (hotelValidation.mobileNoValidation(phoneNo)) {
					if (hotelValidation.userNameValidation(userName)) {
						...
						throw new HotelManagementException("Only Contains Chracter With FirstName and LastName");
					}
				} else {
					throw new HotelManagementException(
							"The Number Starts From 6-9 And It Contains 10 Digit(Number) Only");
				}
	...
	public boolean deleteemployeeHotel(int hotelId) {
		return userDao.deleteemployeeHotel(hotelId);
	}
